    ...
    <AppLayout topbar={
      <TopBar title={
        <span style={{ display:'flex', alignItems:'center', gap:'8px', fontFamily: FONT }}>
          ...
          <span style={{ color:'#111827', fontWeight:'500' }}>Ajouter un équipement</span>
        </span>
      }/>
    }>
      <div style={{ padding:'24px', background:'#f8f9fa', minHeight:'100%' }}>
        <div style={{ maxWidth:'760px', margin:'0 auto', display:'flex', flexDirection:'column', gap:'16px' }}>
          ...
          <div style={{ background:'#fff', border:'1px solid #eaecf0', borderRadius:'12px', padding:'24px', boxShadow:'0 1px 3px rgba(0,0,0,0.04)' }}>
            <h2 style={{ margin:'0 0 20px', fontSize:'15px', fontWeight:'600', color:'#111827', fontFamily: FONT }}>Ajouter un équipement</h2>

            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'14px 16px' }}>

              {/* Nom — 2 col */}
              <F label="Nom" required error={errors.name} col="1 / 3">
                <input className={`form-input${errors.name?' error':''}`} value={form.name} onChange={e=>set('name',e.target.value)} placeholder="ex : PC Bureau DRH-01" autoFocus/>
              </F>
              ...
              <F label="Marque">
                <input className="form-input" value={form.brand} onChange={e=>set('brand',e.target.value)} placeholder="HP, Dell, Lenovo…"/>
              </F>
              <F label="Modèle">
                <input className="form-input" value={form.model} onChange={e=>set('model',e.target.value)} placeholder="ex : EliteBook 840 G8"/>
              </F>

              {/* N° série + Site + Localisation */}
              <F label="Numéro de série">
                <input className="form-input" value={form.serial_number} onChange={e=>set('serial_number',e.target.value)} placeholder="SN-2024-XXXXX"/>
              </F>
              <F label="Site">
              ...
              <F label="Localisation / IP">
                <input className="form-input" value={form.location} onChange={e=>set('location',e.target.value)} placeholder="Bureau 104 / 192.168.x.x"/>
              </F>
              ...
              <F label="Prix d'achat (FCFA)">
                <input type="number" min="0" className="form-input" value={form.purchase_price} onChange={e=>set('purchase_price',e.target.value)} placeholder="0"/>
              </F>
              ...
              <F label="Durée de vie (années)">
                <input type="number" min="0" className="form-input" value={form.lifespan_years} onChange={e=>set('lifespan_years',e.target.value)} placeholder="5"/>
              </F>

              {/* Notes — pleine largeur */}
              <F label="Notes" col="1 / -1">
                <textarea className="form-textarea" style={{ minHeight:'68px' }} value={form.notes} onChange={e=>set('notes',e.target.value)} placeholder="Informations complémentaires…"/>
              </F>
            </div>
          </div>
          ...
          <div style={{ display:'flex', justifyContent:'flex-end', gap:'10px' }}>
            <button className="btn-secondary" onClick={() => navigate('/equipment')}>Annuler</button>
